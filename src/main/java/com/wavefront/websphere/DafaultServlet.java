...
public abstract class DafaultServlet extends HttpServlet {
  ...
  protected static ObjectName getPrefObjectName(MBeanServer server) throws MalformedObjectNameException {
    ObjectName queryName = new ObjectName("WebSphere:type=Perf,*");
    Set<ObjectName> perfNames = server.queryNames(queryName, null);
    return perfNames.stream().findFirst().orElseThrow(() -> new MalformedObjectNameException("type=Perf mBean not found."));
  }
